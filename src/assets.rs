...
pub enum FhirVersion {
    ...
    /// FHIR 4.0.0 (R4).
    V4_0_0,
    /// FHIR 5.0.0 (R5). Generated by `xtask`, not vendored (decisions D5, D9).
    V5_0_0,
}
...
static SCHEMA_JSON: &[&str] = &[
    ...
    include_str!("../assets/schema/fhirpg-5.0.0.sql.json"),
];
...
static TRANSFORM_JSON: &[&str] = &[
    ...
    include_str!("../assets/transform/fhirpg-import-5.0.0.json"),
];
...
static TRANSFORM_CACHE: [OnceLock<TransformMap>; 10] = [const { OnceLock::new() }; 10];

/// One resource-type table map per version, filled on first use.
static RESOURCE_TABLE_CACHE: [OnceLock<BTreeMap<String, String>>; 10] =
...
mod tests {
    ...
    #[test]
    fn version_list_matches_fhirbase_plus_r5() {
        // `AvailableSchemas` (main.go:12-16), plus 5.0.0, which fhirbase never
        // had and which `xtask` generates (decision D4).
        assert_eq!(
            FhirVersion::known(),
            "1.0.2, 1.1.0, 1.4.0, 1.6.0, 1.8.0, 3.0.1, 3.2.0, 3.3.0, 4.0.0, 5.0.0"
        );
    }
    ...
    fn every_schema_asset_parses() {
        for &version in ALL_VERSIONS {
            ...
            assert!(!statements.is_empty(), "FHIR {version} schema is empty");

            if version == FhirVersion::V5_0_0 {
                // Decision D9: the generated schema omits `CREATE EXTENSION
                // pgcrypto` outright. PostgreSQL 18 has `gen_random_uuid()` in
                // core, and the statement fails on an install without contrib.
                assert!(
                    !statements.iter().any(|s| s.contains("CREATE EXTENSION")),
                    "the generated R5 schema must not create an extension"
                );
                assert!(statements[0].contains("resource_status"));
            } else {
                assert!(
                    statements[0].contains("CREATE EXTENSION"),
                    "vendored FHIR {version} opens with the pgcrypto extension"
                );
            }
        }
    }
    ...
    fn every_transform_map_parses_and_validates() {
        ...
        let expected: [(FhirVersion, usize); 10] = [
    ...
    fn vendored_assets_match_their_recorded_checksums() {
        ...
        assert_eq!(
            checked, 21,
            "expected 11 schema assets (9 vendored + generated R5 + functions) and 10 transform maps"
        );
    }
}
