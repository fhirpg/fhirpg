...
| D15 | `--validate` is an opt-in cargo feature carrying `fhir` from crates.io, R5 only, and reports rather than rejects. | The loader is untyped on purpose — it must store data a strict model would refuse — so validation cannot gate the default path. Keeping it a feature means the ~135k lines of generated model are not compiled for a normal build, and taking `fhir` from crates.io rather than a path keeps `cargo publish` unblocked. R5 only because each release in that crate is a separate model behind its own feature; enabling R3 and R4 too would add ~270k more lines for versions most loads do not use. Task T24. |
...
| X13 ✅ | `functions.sql.json`, `fhirbase_delete` | **Fixed in T11c.** The `deleted` CTE copies `status` from the live row instead of writing `'deleted'`, so **no procedure ever produces the `resource_status` enum's `'deleted'` value**. History cannot distinguish a delete from an update. Confirmed against PostgreSQL 18.4: create → update → delete leaves history `['created','updated','updated']`. | Write `'deleted'` for the delete row. Witness test `witness_x13_…` asserts the broken behaviour today and flips when T11c lands. |
| X14 ✅ | `functions.sql.json`, `fhirbase_delete` | **Fixed in T11c.** The procedure inserts into `_history` **twice** — once with the row's existing `txid`, once with the supplied one — and `_history`'s primary key is `(id, txid)`. Supplying a `txid` equal to the row's current one is a unique violation. Not hypothetical: every bulk-loaded row has `txid = 0` (X10), so `fhirpg_delete(rt, id, 0)` on loaded data always fails. | Derive the history rows so the two cannot collide; `RETURNING OLD` (D13) removes the double read that causes it. Witness test `witness_x14_…`. |
| X15 ✅ | `functions.sql.json`, every procedure | **Fixed in T11c.** Procedures build SQL with `format('%s', resource_type)` rather than `%I`. Two consequences: a resource type that lowercases to a reserved word is **unusable** — `fhirpg_create` and `fhirpg_read` on a FHIR `Group` both fail with a syntax error — and resource-derived data is concatenated into a string that `EXECUTE` runs. Demonstrated: feeding `fhirpg_read` a crafted type introduces a join and comments out the query's own alias. This is X2's twin, one layer down. | Use `%I` for every identifier. Witness test `witness_x15_…` covers `Group` and the query-structure change. |
| X16 ✅ | `load.go:169–194` | **Format detection buffers the whole file.** `guessBundleType` calls `bufio.Reader.ReadString('\n')` to get the first line — and a compact FHIR Bundle contains **no newline at all**, so "the first line" is the entire document. Detection allocates the file before the streaming reader ever starts. Measured on a 1 GB single-line Bundle: RSS grew 1,003 MB. Found by T13's memory profile; the port had inherited it. | Read a bounded prefix (1 MiB) and split the first two lines out of that. Same classification for every realistic input, constant memory for all of them. After the fix the same 1 GB bundle grows RSS by 2.0 MB. |
| X17 ✅ | `web/index.html`, `web/app.js` | **The web console ships two third-party trackers.** Google Analytics (`UA-125233238-1`) and Yandex Metrica — the latter configured `webvisor: true`, which **records the session** — on a page that renders patient query results. It also fetches its snippet list from `fhirbase.github.io` on every load, and reports every SQL statement the user runs to Google as an event. Decision D1 removed fhirbase's telemetry from the binary; leaving this in would reinstate it through the UI. Found while rebranding for T20. | Both trackers and the `gtag` event removed; snippets are built into `app.js`. A test asserts the embedded assets contain none of `googletagmanager`, `gtag`, `dataLayer`, `yandex`, `Metrika`, `webvisor`, or the tracking id. |
...
| # | Risk | Mitigation |
| --- | --- | --- |
| ~~R2~~ | ~~**`COPY` with the `resource_status` enum.**~~ **Closed in T25.** Profiling settles it: copy mode spends 7.1 s of 43 s on fhirpg's own CPU, most of that system time, so escaping is a fraction of a fraction. Binary `COPY` would need a per-database enum-OID lookup to recover a few percent. Text format stays. |
| ~~R1~~ | ~~**Streaming a FHIR Bundle's `entry[]` array.**~~ **Closed in T13.** Resolved with a small hand-rolled byte scanner (`src/bundle/scanner.rs`) that navigates to `entry` and hands each element to `serde_json` as a bounded slice — no `DeserializeSeed`, no `struson`. Profiling then found the real leak was in *detection*, not the reader (defect X16). Measured: 1 GB Bundle → 2.0 MB RSS growth, 1 GB NDJSON → 1.9 MB, both at ~1M resources/second. |
...
| ~~R4~~ | ~~**R5 asset correctness.**~~ **Largely closed in T21.** The premise was wrong: the `fhir` crate ships R3, R4 *and* R5 specification JSON, so the generator can be validated against fhirbase's own 3.0.1 and 4.0.0 assets. It reproduces R3 at 126/126 nodes and R4 at 151/155, the four differences being the `Meta` open-type addition between spec 4.0.0 and 4.0.1. Residual risk is only what R5 does that R3 and R4 do not exercise, which T22's hand-verification covers. | Structural self-checks (every `tr/move` target resolves — verified to hold for 4.0.0; every table name unique and quoted); hand-check ~10 diverse resource types against the R5 spec; and diff generated-R5 against vendored-4.0.0 for shared types, where every difference must be explainable by a real R4→R5 change. |
| ~~R5~~ | ~~**`fhir` is a path dependency**~~ **Closed in T21.** The generator reads the specification JSON directly rather than calling `fhir::r5::meta`, so `fhir` is a dependency of nothing. The spec JSON is published by HL7, so the generator is not tied to that checkout either. |
